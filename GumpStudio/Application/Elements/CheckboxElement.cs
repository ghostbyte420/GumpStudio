...
namespace GumpStudio.Elements
{
	...
	public class CheckboxElement : BaseElement, ICSharpExportable
	{
		...
		public string ToCSharpString()
		{
			...
			var state = Checked.ToString().ToLower();

			if (this is RadioElement)
			{
				return $"AddRadio({X}, {Y}, {UnCheckedID}, {CheckedID}, {state}, {buttonName}); // {Name}";
			}

			return $"AddCheck({X}, {Y}, {UnCheckedID}, {CheckedID}, {state}, {buttonName}); // {Name}";
		}
	}
}
