...
namespace GumpStudio.Elements
{
	...
	public class HTMLElement : ResizeableElement, ICSharpExportable
	{
		...
		{
			var bg = ShowBackground.ToString().ToLower();
			var scroll = ShowScrollbar.ToString().ToLower();

			if (TextType == HTMLElementType.Localized)
			{
				return $"AddHtmlLocalized({X}, {Y}, {Width}, {Height}, {CliLocID}, {bg}, {scroll}); // {StringList.ENU.GetString(CliLocID)}";
			}

			return $"AddHtml({X}, {Y}, {Width}, {Height}, \"{HTML.Replace("\"", "\\\"")}\", {bg}, {scroll});";
		}
	}
}
